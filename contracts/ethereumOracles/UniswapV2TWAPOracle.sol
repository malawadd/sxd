...
contract UniswapV2TWAPOracle is Oracle {
     ...
     */
    uint256 public constant MIN_TWAP_PERIOD = 2 minutes;

    // Uniswap stores its cumulative prices in "FixedPoint.uq112x112" format - 112-bit fixed point:
    uint256 public constant UNISWAP_CUM_PRICE_SCALE_FACTOR = 2**112;

    uint256 private constant UINT32_MAX = 2**32 - 1; // Should really be type(uint32).max, but that needs Solidity 0.6.8...
    uint256 private constant UINT224_MAX = 2**224 - 1; // Ditto, type(uint224).max

    IUniswapV2Pair immutable uniswapPair;
    uint256 immutable token0Decimals;
    uint256 immutable token1Decimals;
    bool immutable tokensInReverseOrder;
    uint256 immutable scaleFactor;
     ...
     */
    constructor(
        IUniswapV2Pair uniswapPair_,
        uint256 token0Decimals_,
        uint256 token1Decimals_,
        bool tokensInReverseOrder_
    ) public {
        ...
        (uint256 aDecimals, uint256 bDecimals) = tokensInReverseOrder_
    ...
    {
        (
            CumulativePrice storage olderStoredPrice,
            CumulativePrice storage newerStoredPrice
        ) = orderedStoredPrices();

        uint256 timestamp;
        uint256 priceSeconds;
    ...
    function latestUniswapTWAPPrice() public view returns (uint256 price) {
        ...
        )
    {
        ...
        CumulativePrice storage refPrice = storedPriceToCompareVs(
            timestamp,
            newerStoredPrice
        );
        price = calculateTWAP(
            timestamp,
            priceSeconds,
            uint256(refPrice.timestamp),
            uint256(refPrice.priceSeconds)
        );
    }

    function storeCumulativePrice(
        uint256 timestamp,
        uint256 priceSeconds,
        CumulativePrice storage olderStoredPrice
    ) internal {
        ...
        (olderStoredPrice.timestamp, olderStoredPrice.priceSeconds) = (
            uint32(timestamp),
            uint224(priceSeconds)
        );
    }

    function storedPriceToCompareVs(
        uint256 newTimestamp,
        CumulativePrice storage newerStoredPrice
    ) internal view returns (CumulativePrice storage refPrice) {
        bool aAcceptable = areNewAndStoredPriceFarEnoughApart(
            newTimestamp,
            storedPriceA
        );
        bool bAcceptable = areNewAndStoredPriceFarEnoughApart(
            newTimestamp,
            storedPriceB
        );
        if (aAcceptable) {
    ...
    {
        (olderStoredPrice, newerStoredPrice) = storedPriceB.timestamp >
            storedPriceA.timestamp
            ? (storedPriceA, storedPriceB)
            : (storedPriceB, storedPriceA);
    }

    function areNewAndStoredPriceFarEnoughApart(
        uint256 newTimestamp,
        CumulativePrice storage storedPrice
    ...
    function cumulativePrice()
        private
        view
        returns (uint256 timestamp, uint256 priceSeconds)
    {
        ...
        uint256 uniswapCumPrice = tokensInReverseOrder
            ? uniswapPair.price1CumulativeLast()
            : uniswapPair.price0CumulativeLast();
        priceSeconds =
            uniswapCumPrice.mul(scaleFactor) /
            UNISWAP_CUM_PRICE_SCALE_FACTOR;
    }
    ...
    function calculateTWAP(
        uint256 newTimestamp,
        uint256 newPriceSeconds,
        uint256 oldTimestamp,
        uint256 oldPriceSeconds
    ) private pure returns (uint256 price) {
        price = (newPriceSeconds.sub(oldPriceSeconds)).div(
            newTimestamp.sub(oldTimestamp)
        );
    }
}
